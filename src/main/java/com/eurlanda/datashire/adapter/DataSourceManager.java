...
public class DataSourceManager {
	...
	public DataSourceManager(DbSquid _dataStore) {
        // 使用系统数据源的不需要操作
        if (CommonConsts.DATASHIRE_SERVER_DATABASE_HOST_SYMBOL.equals(_dataStore.getHost())) {
...
//					jdbcURL = "jdbc:hsqldb:hsql://192.168.137.2:9092/" + dataStore.getDb_name();
	...
	public synchronized Connection getDBCPConnection() {
        ...
        // 判断是否是系统的数据库
        if(CommonConsts.DATASHIRE_SERVER_DATABASE_HOST_SYMBOL.equals(dataStore.getHost())) {
            // 系统数据源
