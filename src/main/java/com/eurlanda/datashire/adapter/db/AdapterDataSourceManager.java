...
public class AdapterDataSourceManager {
    ...
    public static INewDBAdapter getAdapter(DBConnectionInfo conInfo) throws Exception {
        if (conInfo.getHost() == null) {
            throw new SystemErrorException(MessageCode.ERR_CONNECTION_NULL, "Connection对象为空");
        }
        if(conInfo.getDbType() == DataBaseType.CASSANDRA) {
            if(conInfo.getKeyspace() == null) {
                throw new SystemErrorException(MessageCode.ERR_CONNECTION_NULL, "Connection对象为空");
            }
        } else {
            if(conInfo.getDbName() == null) {
                throw new SystemErrorException(MessageCode.ERR_CONNECTION_NULL, "Connection对象为空");
            }
        }
