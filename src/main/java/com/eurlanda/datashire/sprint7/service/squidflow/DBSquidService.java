 ...
 *
 */
public class DBSquidService extends AbstractRepositoryService{
	
	public DBSquidService(String token) {
		super(token);
	}
	protected DBSquidService(IRelationalDataManager adapter){
		super(adapter);
	}
	protected DBSquidService(String token, IRelationalDataManager adapter){
		super(token, adapter);
	}
